...
public class MyWekaUtils {
    ...
    public static double train(String arffData, int option) throws Exception {
    ...
    public static double train(String arffData, int option, String saveDir) throws Exception {
        StringReader strReader = new StringReader(arffData);
        Instances instances = new Instances(strReader);
        strReader.close();
        instances.setClassIndex(instances.numAttributes() - 1);
        ...
        weka.core.SerializationHelper.write(saveDir, classifier);

        return eval.pctCorrect();
    }

    public static double[] classifyInstance(String arffData, String[] classes, Classifier classifier) throws Exception {
        StringReader strReader = new StringReader(arffData);
        ...
        for (int i = 0; i < classes.length; i++) {
            classVal.add(classes[i]);
        }
        unlabeled.deleteAttributeAt(unlabeled.numAttributes() - 1);
        unlabeled.insertAttributeAt(new Attribute("pattern", classVal), unlabeled.numAttributes());
        unlabeled.setClassIndex(unlabeled.numAttributes() - 1);

        double result = classifier.classifyInstance(unlabeled.instance(0));
        double[] distribution = classifier.distributionForInstance(unlabeled.instance(0));
        Log.d("MyWekaUtils", "classifyInstance: " + result);
        String resultString = "";
        for (double d : distribution) {
            resultString += d + " ";
        }
        Log.d("MyWekaUtils", "classifyInstance: " + resultString);
        return distribution;
    }
    ...
    public static String csvToArff(String[][] csvData, int[] featureIndices) throws Exception {
        ...
        Log.println(Log.DEBUG, "ARFF", sb.toString());
        return sb.toString();
    }
